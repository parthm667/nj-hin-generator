...
The cache is under `/srv/data/raw/njdot-dashboard`; acquisition and import reports and the staging database are under `/srv/data/processed`. Budget disk space for the full CSV transfer, filtered CSV, staging database, and database updates. The observed full export was about 1.12 GB; compressed network size or a small preview is not the storage requirement.
